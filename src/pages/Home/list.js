...
import { Row, Col, List, Card, Button, Form, Select, DatePicker, Typography, Image, Statistic, Tag } from 'antd'

export default function ListPage({ path, loading, pagination, sorted, onLoadMore, onFilter }) {
  const loadMore =
          ...
          <List
            ...
            renderItem={item => {
              ...
              const labelGenres = item.genre_ids.map(id => {
                const genre = pagination.genres.filter(item => item.id === id)
                return <Tag key={id}>{genre[0].name}</Tag>
              })
              const description = (
                <>
